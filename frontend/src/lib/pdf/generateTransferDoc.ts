...
export type FluxPdf = 'SELEC_UPSEMCL' | 'UPSEMCL_MULT' | 'MULT_QUOT'
...
export interface TransferDocData {
  ...
  /** Utilisé pour l'en-tête MULT_QUOT — nom complet de l'org émettrice */
  orgEmetteurNom?:    string
  orgEmetteurRegion?: string
}
...
export function generateNumero(transfertId: number, flux?: FluxPdf): string {
  const year = new Date().getFullYear()
  const seq  = String(transfertId).padStart(3, '0')
  if (flux === 'MULT_QUOT') return `N° ${seq}/${year}`
...
function drawIsraHeader(doc: jsPDF, ML: number, TW: number, Y: number): void {
...
function drawMultHeader(doc: jsPDF, ML: number, Y: number, orgNom: string, orgRegion?: string): void {
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  doc.setTextColor(...DARK)
...
export function generateTransferDoc(data: TransferDocData): { blobUrl: string; filename: string } {
  ...
  if (flux === 'MULT_QUOT') {
    ...
    drawMultHeader(doc, ML, Y, data.orgEmetteurNom || data.expediteur.roleLabel, data.orgEmetteurRegion)
  ...
  /* Bloc droit — label selon flux, sans cercle */
  const sigTopY = Y - 23
  const sigLabel = flux === 'MULT_QUOT'
    ? 'Le Multiplicateur'
    : flux === 'SELEC_UPSEMCL'
      ? 'Le Sélectionneur'
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text(sigLabel, sigRightX, sigTopY)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.text('Signature :', sigRightX, sigTopY + 8)
  doc.setDrawColor(...GRAY)
  doc.setLineWidth(0.3)
  doc.line(sigRightX + 22, sigTopY + 8, sigRightX + 62, sigTopY + 8)
  ...
  const footer = flux === 'MULT_QUOT'
    ? (data.orgEmetteurNom || 'Multiplicateur agréé') + ' — Sénégal'
