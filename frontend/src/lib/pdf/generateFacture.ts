...
export function generateFacture(data: FactureData): FactureResult {
  ...
  /* numéro facture + date — bloc droit */
  const RX = ML + TW
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  ...
  /* ═══ CONDITIONS ═══ */
  const conditions = data.conditions || "Paiement 30 jours — Virement ou chèque à l'ordre de ISRA/CNRA"
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(...MUTED)
  ...
  doc.setFontSize(9)
  doc.setTextColor(...DARK)
  doc.text('LE VENDEUR', ML + sigW / 2, Y, { align: 'center' })
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8.5)
  Y += 7
  doc.text('Nom : ___________________________________________', ML, Y)
  Y += 7
  doc.text('Signature & Cachet :', ML, Y)
  doc.setDrawColor(...GRAY)
  doc.setLineWidth(0.25)
  doc.circle(ML + sigW - 12, Y + 8, 10, 'S')

  const s2X = ML + sigW + 8
  const sigTopY = Y - 14
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text("BON POUR ACCORD — L'ACHETEUR", s2X + sigW / 2, sigTopY, { align: 'center' })
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8.5)
  doc.text('Nom : ___________________________________________', s2X, sigTopY + 7)
  doc.text('Signature & Cachet :', s2X, sigTopY + 14)
  doc.setDrawColor(...GRAY)
  doc.setLineWidth(0.25)
  doc.circle(s2X + sigW - 12, sigTopY + 22, 10, 'S')
